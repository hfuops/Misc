...
class ChefAPI
  ...
  def initialize(opts={})
    ...
    # Don't know how to use OpenSSL::SSL::VERIFY_PEER, I check .crt and its associated RSA,but failed.
    # So don't set ssl_insecure = false unless you figure it out.
...
chef = ChefAPI.new(
  :server       => 'chef.xxx.com',
  :port         => 9443,
  :ca_file      => '/root/.chef/trusted_certs/xxx.crt',
  :client_name  => 'opscode',
  :key_file     => '/etc/chef/opscode.pem',
  :ssl_insecure => true
)

puts chef.get_request('/organizations/xxx/nodes', "") 
